...
module.exports = (req, res) => {
    applyCors(req, res, async () => {
        ...
        try {
            ...
            // 2️⃣ Recupera tipo competizione per capire come estrarre i giocatori
            ...
            const isGroupKnockout = ['group_knockout', 'group_knockouts'].includes((compData?.type || '').toLowerCase());

            let qualified = [];

            if (isGroupKnockout) {
                // Recupera dai gironi i primi e secondi classificati
                ...
                (players || []).forEach(p => {
                    if (p.ranking === 1) firsts.push(p);
                    else if (p.ranking === 2) seconds.push(p);
                });

                // Incrocia i primi con i secondi classificati di gironi possibilmente diversi
                for (let i = 0; i < Math.max(firsts.length, seconds.length); i++) {
                    if (firsts[i]) {
                        qualified.push({
                            id: firsts[i].player_id,
                            nickname: firsts[i].nickname,
                            imageUrl: firsts[i].image_url,
                        });
                    }
                    if (seconds.length > 0) {
                        const secondIdx = (i + Math.floor(seconds.length / 2)) % seconds.length;
                        if (seconds[secondIdx]) {
                            qualified.push({
                                id: seconds[secondIdx].player_id,
                                nickname: seconds[secondIdx].nickname,
                                imageUrl: seconds[secondIdx].image_url,
                            });
                        }
                    }
                }
            } else {
                // Eliminazione diretta: prendi TUTTI i giocatori iscritti alla competizione
                ...
                qualified = (players || [])
                    .map((p) => ({
                        ...
                        imageUrl: p.player?.image_url,
                    }))
                    .filter(p => p.id);
                
                // Ordina per ID per mantenere i seed deterministici e non rompere la struttura
                qualified.sort((a, b) => a.id - b.id);
            }
...
function buildKnockoutStructure(players) {
    ...
    // 🔹 Mescola i giocatori in modo casuale (Fisher-Yates)
