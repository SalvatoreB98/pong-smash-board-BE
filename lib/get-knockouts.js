...
function buildKnockoutStructure(players) {
    ...
    for (let i = 0; i < fullPlayers.length; i += 2) {
        const player1 = fullPlayers[i];
        const player2 = fullPlayers[i + 1] ?? null;

        current.push({
            player1,
            player2,
            isBye: !player1 || !player2, // 🔹 SOLO QUI segniamo i bye
        });
    }
    ...
    for (let r = 0; r < numRounds; r++) {
        const matches = current.map((m, idx) => ({
            ...
            player1: m.player1,
            player2: m.player2,
            isBye: m.isBye ?? false, // 🔸 i successivi non hanno bye veri
        }));
        ...
        for (let i = 0; i < matches.length; i += 2) {
            next.push({ player1: null, player2: null, isBye: false });
        }
